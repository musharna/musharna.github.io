...
**Damage moves the measurement, but does not make it.** Solidity, the filter the pipeline already uses to reject torn leaves, still tracks PC1 among the leaves that passed it (**r = 0.360**), and within species too (mean 0.308): a torn leaf reads as narrower. Residualising it out costs little, η² **0.493 → 0.434** and accuracy **0.372 → 0.360**, and damage carrying no shape information at all classifies at **0.195** against a 0.191 baseline. The signal is shape, not preservation. Worth stating anyway: _L. glandulosa_ is both the **most damaged** species and the outlier carrying the agreement above.
