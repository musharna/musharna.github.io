...
A second, related piece of work came out of the same lab project — the [digital reconstruction of _Lobelia_ silhouettes]({{ '/projects/LobeliaSilhouettes/' | relative_url }}), whole-plant outlines drawn from restored herbarium specimens. The two are easy to conflate, because both end in a plant reduced to an outline. They are not the same thing: that one is reconstruction for depiction, this one is segmentation for measurement.
...
4. **Segment.** A separate instance-segmentation model ran on the crops, producing the per-leaf masks the morphometrics depend on — and the leaf outlines shown above.
